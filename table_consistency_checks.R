...
assert_size_comparison <- function(df1, direction, df2){
  msg = glue::glue("num rows of df1 = {num_row(df1)} is not",
...
assert_size_uniques <- function(df, col_name, direction, size){
  ...
  msg = glue::glue("number of uniques in {cc} of df = {num_unique_entries(df, col_name)}",
...
assert_size_missing <- function(df, col_name, direction, size){
  cc = paste0(col_name, collapse = ", ")
  msg = glue::glue("number of missings in {cc} of df = {num_missing_entries(df, col_name)}",
                   " is not {direction} {size}")
...
check_no_date_overlap <- function(df, start_date, end_date, group_by_cols){
  ...
  df = df %>%
    dplyr::rename(sd_tmp = !!dplyr::sym(start_date),
  ...
  if(common_start > 0){ return( FALSE) }
  ...
  number_overlaps = df %>%
    ...
    dplyr::filter(sd_tmp_x <= ed_tmp_y, # overlap check
           sd_tmp_y <= ed_tmp_x,
