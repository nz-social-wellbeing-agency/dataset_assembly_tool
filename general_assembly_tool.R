...
general_data_assembly_tool <- function(
  ...
  population_required_cols = c("database_name", "schema_name", "table_name", "identity_column",
                               "label_identity", "summary_period_start_date", "summary_period_end_date", "label_summary_period")
  measure_required_cols = c("database_name", "schema_name", "table_name", "identity_column",
                            "measure_period_start_date", "measure_period_end_date", "label_measure", "value_measure",
                            "measure_summarised_by", "proportional")
  input_population_and_period_table = select(input_population_and_period_table, population_required_cols)
  input_measures_to_assemble_table = select(input_measures_to_assemble_table, measure_required_cols)
  ...
  if(!control_run_checks_only){
    assemble_output_table(input_population_and_period_table, input_measures_to_assemble_table,
...
validate_control_tables <- function(population_table, measure_table, control_verbose){
  ...
  column_reqs = data.frame(Database_name             = c("sql only", TRUE, FALSE),
  ...
  for(col in colnames(column_reqs))
    for(row in 1:nrow(population_table)){
      # required delimiters
      sql_check = as.logical(column_reqs["sql", col]) & is_delimited(population_table[[row, col]], "[]")
      txt_check = as.logical(column_reqs["txt", col]) & is_delimited(population_table[[row, col]], "\"")
      
      if(!sql_check & !txt_check){
        any_check_failed = TRUE
        warning(sprintf("unaccepted input in population control: column %s, row %d - acceptable values are %s",
  ...
  for(col in colnames(column_reqs))
    for(row in 1:nrow(measure_table)){
      # required delimiters
      sql_check = as.logical(column_reqs["sql", col]) & is_delimited(measure_table[[row, col]], "[]")
      txt_check = as.logical(column_reqs["txt", col]) & is_delimited(measure_table[[row, col]], "\"")
      
      if(!sql_check & !txt_check){
        any_check_failed = TRUE
        warning(sprintf("unaccepted input in measure control: column %s, row %d - acceptable values are %s",
                        col, row, column_reqs["accepts",col]))
      }
  ...
  # acceptable and default values
  column_reqs = list(Measure_summarised_by = c("SUM", "COUNT", "EXISTS", "MIN", "MAX", "DURATION", "HISTOGRAM"),
  ...
  for(col in names(column_reqs))
    for(row in 1:nrow(measure_table)){
      check = tolower(measure_table[[row, col]]) %in% tolower(column_reqs[[col]])
    ...
    for(col in col_names){
      ...
      if("measure_summarised_by" %in% col_names & col == "value_measure" & is_delimited(this_control_value, "[]")){
        ...
        if(summary_type %in% c("SUM", "MIN", "MAX")){
          ...
          assert(is.numeric(var_example) | is.logical(var_example),
...
assemble_output_table <- function(population_table, measure_table,
  ...
  # connect to db
  db_con = create_database_connection(database = substr(output_database, 2, nchar(output_database) -1))
  
  # delete
  if(control_overwrite_output_table)
    delete_table(db_con, output_database, output_schema, output_table)
  # required table
  output_columns = list(identity_column = "[int] NOT NULL",
  ...
  for(row_p in 1:nrow(population_table)){
    ...
    for(row_m in 1:nrow(measure_table)){
      ...
      calculation = handle_summary_case(summary_type = measure_table[[row_m, "measure_summarised_by"]],
      ...
      # prepare query
      sql_query <- dbplyr::build_sql(con = db_con,
                                     sql(glue::glue(
                                       "SELECT {optional_top}\n",
      ...
      close_database_connection(db_con)
      run_time_inform_user(sprintf("completed population %3d of %3d, measure %4d of %4d",
  ...
  #### tidy up ----
  #
  # compressing table and creating clustered index
  # were removed from here so that they are a deliberate choice by the user
}
