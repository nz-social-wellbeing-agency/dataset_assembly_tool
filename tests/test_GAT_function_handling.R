...
test_that("missing values in table errors",{
  ...
  expect_error(validate_control_tables(tmp1b, tmp2, control_verbose = "none"))
})
...
for(col in colnames(accidents))
  if(is.character(accidents[[1,col]])){   accidents[,col] = gsub("[\"']", "", accidents[[col]])   }
for(col in colnames(benefit_payment))
  if(is.character(benefit_payment[[1,col]])){   benefit_payment[,col] = gsub("[\"']", "", benefit_payment[[col]])   }
for(col in colnames(project_population))
  if(is.character(project_population[[1,col]])){   project_population[,col] = gsub("[\"']", "", project_population[[col]])   }
...
test_that("numeric operations fail on non-numeric columns",{
  ...
  tmp2b = tmp2; tmp2b[tmp2b$table_name == "[tmp_benefit_payment]", "table_name"] = "[tmp_benefit_payment_flaw]"
...
test_that("assembly is correct",{
  ...
  for(col in colnames(output_example))
    if(is.character(output_example[[1,col]])){   output_example[,col] = gsub("[\"']", "", output_example[[col]])   }
  ...
  for(col in colnames(actual_output))
    if(is.character(actual_output[[1,col]])){   actual_output[,col] = gsub("[\"']", "", actual_output[[col]])   }
