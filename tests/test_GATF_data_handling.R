...
test_that("summary case gives required outputs",{
  ...
  list_out = handle_summary_case("SUM", FALSE, 'm_label', 'm_value',
...
test_that("proportional calculates correctly",{
  ...
  expected_value = paste0("1.0 * (1+DATEDIFF(DAY, IIF(m_start < p_start, p_start, m_start), IIF(m_end < p_end, m_end, p_end)))",
